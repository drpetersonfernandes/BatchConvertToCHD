...
internal sealed record CueFileReference(string ReferencedName, string? ResolvedName, string FullPath, string? TrackType, bool WasNameCorrected = false)
{
    /// <summary>True when a file with this name (possibly case/padding-resolved) exists next to the cue.</summary>
    public bool IsResolved => ResolvedName is not null;

    /// <summary>Full path of the resolved on-disk file, or <see cref="FullPath"/> when unresolved.</summary>
    public string ResolvedFullPath => ResolvedName is null ? FullPath : Path.Combine(Path.GetDirectoryName(FullPath) ?? string.Empty, ResolvedName);
}
