...
public class CueNormalizerTests : IDisposable
{
    ...
    public async Task NormalizeAsyncUnquotedFileLineIsRewrittenQuoted()
    {
        ...
        Assert.Contains("FILE \"track1.bin\" BINARY", result.CanonicalCueText, StringComparison.Ordinal);
    ...
    public async Task NormalizeAsyncZeroPaddingMismatchResolvesToOnDiskName()
    {
        ...
        Assert.Contains("FILE \"Game (Track 2).bin\" BINARY", result.CanonicalCueText, StringComparison.Ordinal);
        Assert.Contains("FILE \"Game (Track 1).bin\" BINARY", result.CanonicalCueText, StringComparison.Ordinal);
    }
    ...
    public async Task NormalizeAsyncKoreanCp949CueResolvesAndRewrites()
    {
        ...
        Assert.Contains($"FILE \"{koreanName}\" BINARY", result.CanonicalCueText, StringComparison.Ordinal);
    }
    ...
    public async Task NormalizeAsyncTransformRewritesFileLineAndType()
    {
        ...
        var cuePath = CreateFile("game.cue", "FILE \"track1.mp3\" MP3\r\n  TRACK 01 AUDIO\r\n    INDEX 01 00:00:00");

        var result = await CueNormalizer.NormalizeAsync(
            cuePath, CancellationToken.None,
            static r => string.Equals(r.TrackType, "MP3", StringComparison.Ordinal) ? ("track01.wav", "WAVE") : null);

        Assert.True(result.NeedsRewrite);
        Assert.Contains("FILE \"track01.wav\" WAVE", result.CanonicalCueText, StringComparison.Ordinal);
    ...
    public async Task WriteCanonicalCueAsyncWritesUtf8CrLfContent()
    {
        ...
        Assert.Contains("FILE \"track1.bin\" BINARY\r\n  TRACK 01 MODE2/2352\r\n    INDEX 01 00:00:00\r\n", written, StringComparison.Ordinal);
